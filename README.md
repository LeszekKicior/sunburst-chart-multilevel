...
| <b>countDirectly</b>([<i>boolean</i>]) | Getter/setter for whether to use the node's size value directly, instead of summing it with its children. Use this if your data already contains cumulative values for parent nodes. | `false` |
...
| <b>maxRank</b>([<i>number</i>]) | Getter/setter for the maximum `level` to show. Nodes where `level` is equal or greater than `maxRank` are hidden. | - |
