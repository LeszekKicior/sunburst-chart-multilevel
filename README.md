...
| <b>nodeId</b>([<i>string</i> or <i>fn</i>]) | Getter/setter for a node object unique ID accessor. If provided, IDs are used to bind data to DOM elements, ensuring smooth transitions when nodes are added or removed from the hierarchy. Supports either a <i>string</i> indicating the object's property name, or a `function(node)` which should return a unique ID. | `index` |
...
| <b>valueCountMethod</b>([<i>string</i>]) | Getter/setter for the method used to compute the segment sizes from the nodes' `size` values. Supports: `node-sum` (default): segment size is the sum of its own value plus all its descendants; `direct`: segment size is based only on its own value; `leaf-only`: segment size is the sum of its leaf descendants' values. See [here for an example](https://vasturiano.github.io/sunburst-chart/example/value-count-methods) of the different methods side-by-side. | `node-sum` |
| <b>level</b>([<i>number</i> or <i>fn</i>]) | Getter/setter for a node object's absolute radial level accessor, used to manually define the outer level of each arc. Child node levels must be greater than their parent level, but can increase by any amount (for example `10`, `20`, `50`). If unspecified or invalid, `levelSpan` is used instead. | - |
...
| <b>fontSize</b>([<i>number</i>]) | Getter/setter for the base font size (in px) used for segment labels. Labels that do not fit will be automatically reduced per segment down to a minimum size (`MIN_FONT_SIZE = 8`), and hidden if they still don't fit. | `12` |
